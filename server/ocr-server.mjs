...
const OPENAI_BASE_URL = normalizeBaseUrl(getEnv("OPENAI_BASE_URL", "https://api.openai.com/v1"));
const OPENAI_TIMEOUT_MS = Number(getEnv("OPENAI_TIMEOUT_MS", String(60 * 1000)));

// Environment Checks
if (OCR_PROVIDER === "ollama" && !OLLAMA_MODEL) { console.error("Missing env OLLAMA_MODEL"); process.exit(1); }
if (OCR_PROVIDER === "gemini" && !GEMINI_API_KEY) { console.error("Missing env GEMINI_API_KEY"); process.exit(1); }
if (OCR_PROVIDER === "lmstudio" && !LMSTUDIO_MODEL) { console.error("Missing env LMSTUDIO_MODEL"); process.exit(1); }
if (OCR_PROVIDER === "openai" && !OPENAI_API_KEY) { console.error("Missing env OPENAI_API_KEY"); process.exit(1); }
...
function buildPrompt(mode) {
  ...
  if (mode === "markdown") return base + "Return ONLY Markdown/plain text (no code fences, no extra explanations).\n";
  
  // Dùng chung cho "json" và "both"
  return base + "Return ONLY valid JSON (no markdown wrappers, no extra text) with fields:\n- markdown: string\n- full_text: string\n- blocks: array of {text, x, y, width, height} (0-100)\nIf bounding boxes are uncertain, still return approximate values.\n";
}
...
function postProcessMarkdown(markdown) {
  ...
  if (OCR_MARKDOWN_STYLE === "clean") {
    lines = lines.filter((l) => {
      const t = l.trim();
      if (!t) return true;
      if (/^\d+\s*-\s*Text\s*$/i.test(t) || /^\d+\s*-\s*Marginalia\s*$/i.test(t) || /^Marginalia\s*$/i.test(t) || /^Text\s*$/i.test(t) || /^Sheet\s+\d+(\s*\/\s*\d+)?\s*$/i.test(t)) return false;
      return true;
    });
  }
  return lines.join("\n").replace(/\n{3,}/g, "\n\n").trim();
...
// --------------------------------------------------------------------------------------
async function callOpenAIFormat({ baseUrl, apiKey, model, imageBase64, mimeType, prompt, timeoutMs }) {
  const url = `${baseUrl}/chat/completions`;
  const payload = {
    model,
    messages: [
      { role: "system", content: "You are an expert OCR assistant. Extract text exactly as seen in the image." },
      { role: "user", content: [
  ...
  if (prompt.includes("ONLY valid JSON")) payload.response_format = { type: "json_object" };

  const headers = { "Content-Type": "application/json" };
  if (apiKey) headers["Authorization"] = `Bearer ${apiKey}`;

  try {
    const r = await httpRequestJson(url, { headers, body: JSON.stringify(payload), timeoutMs });
    const raw = r.text;
    if (!r.ok) {
    ...
    let data;
    try { data = JSON.parse(raw); } catch { return { ok: false, status: 502, raw: "Invalid JSON from API" }; }

    const contentText = data?.choices?.[0]?.message?.content;
    if (typeof contentText !== "string" || contentText.length === 0) return { ok: false, status: 502, raw: "Empty response from API" };

    return { ok: true, status: 200, contentText };
  } catch (e) {
    return { ok: false, status: 502, raw: e instanceof Error ? e.message : String(e) };
  }
}
...
async function callGemini({ imageBase64, mimeType, prompt, responseMimeType }) {
  ...
  try {
    ...
    let r;
    try {
      r = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), signal: controller.signal });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (msg.toLowerCase().includes("aborted")) return { ok: false, status: 504, raw: `Gemini request timed out` };
      return { ok: false, status: 502, raw: msg };
    }

    const raw = await r.text();
    if (!r.ok) { console.error("[ocr-server] gemini error", r.status, safeSnippet(raw)); return { ok: false, status: r.status, raw }; }

    let data;
    try { data = JSON.parse(raw); } catch { return { ok: false, status: 502, raw: "Invalid JSON from Gemini" }; }

    const contentText = data?.candidates?.[0]?.content?.parts?.find((p) => typeof p?.text === "string")?.text;
    if (!contentText) return { ok: false, status: 502, raw: "Empty response from Gemini" };
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/api/ocr") {
    try {
      ...
      if (!imageBase64 || typeof imageBase64 !== "string") return sendJson(res, 400, { error: "imageBase64 is required" });

      // Ưu tiên mode json/both nếu Provider hỗ trợ
      const prompt = ["gemini", "lmstudio", "openai"].includes(OCR_PROVIDER) 
        ? buildPrompt(OCR_MODE === "json" || OCR_MODE === "both" ? "both" : "markdown") 
      ...
      if (OCR_PROVIDER === "gemini") {
        ...
        content = GEMINI_BBOX_JSON 
          ? out.contentText 
          : JSON.stringify({ markdown: out.contentText, full_text: out.contentText, blocks: [] });
      } 
      
      // 2. Tuyến xử lý GỘP cho OpenAI / LM Studio / Ollama (chế độ OpenAI)
      else if (OCR_PROVIDER === "openai" || OCR_PROVIDER === "lmstudio" || (OCR_PROVIDER === "ollama" && OLLAMA_API === "openai")) {
        let baseUrl, apiKey, model, timeoutMs;
        
        if (OCR_PROVIDER === "openai") {
          baseUrl = OPENAI_BASE_URL; apiKey = OPENAI_API_KEY; model = OPENAI_MODEL; timeoutMs = OPENAI_TIMEOUT_MS;
        } else if (OCR_PROVIDER === "lmstudio") {
          baseUrl = LMSTUDIO_BASE_URL; apiKey = ""; model = LMSTUDIO_MODEL; timeoutMs = LMSTUDIO_TIMEOUT_MS;
        } else {
          baseUrl = OLLAMA_OPENAI_BASE_URL; apiKey = "ollama"; model = OLLAMA_MODEL; timeoutMs = OLLAMA_TIMEOUT_MS;
        }

        const out = await callOpenAIFormat({ baseUrl, apiKey, model, imageBase64, mimeType, prompt, timeoutMs });
        if (!out.ok) return sendJson(res, out.status, { error: out.raw });
      ...
      // 3. Tuyến xử lý cho Ollama (chế độ Native cũ)
        ...
        const doOllama = async () => {
          const payload = { model: OLLAMA_MODEL, messages: [{ role: "user", content: prompt, images: [imageBase64] }], stream: false, options: { temperature: 0 } };
          return await httpRequestJson(`${OLLAMA_BASE_URL}/api/chat`, { headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), timeoutMs: OLLAMA_TIMEOUT_MS });
        };

        let r;
        try { r = await doOllama(); } catch (e) { r = await doOllama(); } // Retry once
        if (!r.ok) return sendJson(res, r.status || 502, { error: r.text || `Ollama error ${r.status}` });

        let data;
        try { data = JSON.parse(r.text); } catch { return sendJson(res, 502, { error: "Invalid JSON from Ollama" }); }
        content = data?.message?.content;
      }

      if (!content) return sendJson(res, 502, { error: "Empty response from OCR provider" });
      ...
      try {
        ...
        return sendJson(res, 200, { markdown: cleaned, full_text: cleaned, blocks: [], warning: "Model did not return JSON; returned plain text/markdown instead." });
      }

      return sendJson(res, 200, {
        markdown: postProcessMarkdown(typeof parsed?.markdown === "string" ? parsed.markdown : ""),
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  if (OCR_PROVIDER === "ollama") console.log(`[ocr-server] ollama base: ${OLLAMA_BASE_URL} | model: ${OLLAMA_MODEL}`);
  else if (OCR_PROVIDER === "gemini") console.log(`[ocr-server] gemini model: ${GEMINI_MODEL}`);
  else if (OCR_PROVIDER === "lmstudio") console.log(`[ocr-server] lmstudio base: ${LMSTUDIO_BASE_URL} | model: ${LMSTUDIO_MODEL}`);
  else if (OCR_PROVIDER === "openai") console.log(`[ocr-server] openai base: ${OPENAI_BASE_URL} | model: ${OPENAI_MODEL}`);
});